...
const Burger: React.FC<BurgerProps> = ({ isVisible, onClickBurger, onClickCart }) => {
  const openCart = () => {
    onClickCart(true);
    onClickBurger(false);
  };
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            className="flex flex-col fixed top-0 left-0 bg-white h-full w-full z-30">
            <div className="flex items-center justify-between p-2 bg-lightGreen">
              <div className="flex items-center">
                <img src="assets/logo.png" alt="logo" />
                <div className="ml-3 md:block">
                  <h3 className="font-bold text-xl lg:text-2xl uppercase leading-6">
                    React Sneakers
                  </h3>
                </div>
              </div>
              <Button className="border-[#666666]" onClick={() => onClickBurger(false)} small>
                <svg
                  width="10"
                  height="10"
                  viewBox="0 0 10 10"
            ...
            <ul className="flex flex-col h-full text-center justify-end">
              <li className="bg-slate-100 grow border flex items-center justify-center">
                <Link onClick={() => onClickBurger(false)} to="/">
                  <svg
              ...
              <li className="bg-slate-100 grow border flex items-center justify-center">
                <Link onClick={() => onClickBurger(false)} to="/liked">
                  <svg
                    width="21"
                    height="19"
                    className="inline mr-1"
                  ...
                  <span>Сипсок бажань</span>
                </Link>
              </li>
              <li className="bg-slate-100 grow border flex items-center justify-center">
                <Link onClick={() => onClickBurger(false)} to="/purchase">
                  <svg
                    className="inline mr-1"
                    width="20"
                    height="20"
