...
const SortPopup: React.FC<SortPopupProps> = ({ sortValue }) => {
  ...
  return (
    <div ref={sortRef} className="relative">
      <Button onClick={changeVisiblePopup} className="flex items-center p-1 sm:p-2" small>
        <div className="mr-1">
          <svg className="w-6 h-6 md:w-8 md:h-8" viewBox="0 0 24 24" fill="gray">
        ...
        <div className="text-left">
          <h3 className="uppercase text-[10px] sm:text-xs">Сортування</h3>
          <div className="text-[10px] sm:text-xs font-light">{sortValue.name}</div>
        </div>
      </Button>
      <AnimatePresence>
        {visiblePopup && (
          <motion.div
            ...
            className="absolute bg-white top-11 sm:top-14 left-0 z-10 rounded-lg shadow-xl">
